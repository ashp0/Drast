...
- `use std` enables the default runtime include and standard helper functions.
- `use no_runtime` emits standard library includes but skips `runtime/drast_runtime.hpp`.
...
Emits the runtime include, a quoted C++ include for `native_add.hpp`, and loads `math.drast` from the same directory.
...
Emits `drast::println("hello")` and `add(1, 2)`.
...
- `text.lineCount` -> `drast::line_count(text)`
- `text.splitWhitespace` -> `drast::split_whitespace(text)`
- `text.trim` -> `drast::trim(text)`
- `text.asInt` -> `drast::parse_int(text)`
- `optional.value` -> `.value()`
- `text.lowercase` -> `drast::lowercase(text)`
- `map.keys` -> `drast::map_keys(map)`
- `map.values` -> `drast::map_values(map)`
- `.contains`, `.startsWith`, `.endsWith`, `.find`, `.replace`, `.split`, `.get`, `.set`, `.clear`, `.removeAt`, `.remove`, `.substring`, `.valueOr` map to runtime helpers or STL calls.
...
If `main` has no parameters, `use std` is active, and `no_runtime` is not active, emitted C++ `main` receives `int argc, char **argv` and calls `drast::setArgs(argc, argv)`.
